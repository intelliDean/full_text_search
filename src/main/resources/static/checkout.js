...
// Define the items the customer wants to buy
const request = {
    amount: amount,
    email: email,
    productName: productName
...
// Initialize Stripe Elements and fetch a payment intent
async function initialize() {
    try {
        const response = await fetch("/create-payment-intent", {
            method: "POST",
...
// Fetch and display payment intent status
async function checkStatus() {
    const clientSecret = new URLSearchParams(window.location.search).get(
        "payment_intent_client_secret"
    );
    ...
    try {
        ...
        console.error("Payment intent status check error:", error);
    }
}
...
// Show a spinner during payment submission
function setLoading(isLoading) {
